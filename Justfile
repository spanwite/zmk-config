...
config := '$USERPROFILE/Code/areas/firmwares/zmk-config'
boards := config / 'boards'
combos := config / 'keymap/combos.dtsi'
...
_parse_combos:
    ...
    if [[ -f $cconf ]]; then
        ...
        sed -Ei "/CONFIG_ZMK_COMBO_MAX_COMBOS_PER_KEY/s/=.+/=$count/" "{{ boards }}"/*.conf
        ...
        sed -Ei "/CONFIG_ZMK_COMBO_MAX_KEYS_PER_COMBO/s/=.+/=$count/" "{{ boards }}"/*.conf
...
_parse_targets $expr:
    ...
    echo "$(yq -r "$filter" {{ config / 'build.yaml' }} | grep -v "^," | grep -i "${expr/#all/.*}")"
...
_build_single $board $shield *west_args:
    ...
    west build -s zmk/app -d "$build_dir" -b $board {{ west_args }} -- \
        -DZMK_CONFIG="{{ boards }}" ${shield:+-DSHIELD="$shield"}
