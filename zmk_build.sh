...
# This is a modified urob's script
...
[[ -z $OUTPUT_DIR ]] && OUTPUT_DIR="$USERPROFILE/Downloads"
[[ -z $LOG_DIR ]] && LOG_DIR="/tmp"

[[ -z $HOST_ZMK_DIR ]] && HOST_ZMK_DIR="$HOME/zmk"
[[ -z $HOST_CONFIG_DIR ]] && HOST_CONFIG_DIR="$USERPROFILE/Code/Github/spanwite/zmk-config"
...
# usage: compile_zmk board shield
compile_zmk () {
    BUILD_DIR="${2}-${1}_$SUFFIX"
    LOGFILE="$LOG_DIR/zmk_build_$2-$1.log"
    [[ $MULTITHREAD = "true" ]] || echo -en "\n$(tput setaf 2)Building $2... $(tput sgr0)"
    [[ $MULTITHREAD = "true" ]] && echo -e "$(tput setaf 2)Building $2... $(tput sgr0)"
    $DOCKER_PREFIX west build -d "build/$BUILD_DIR" -b $1 $WEST_OPTS \
        -- -DSHIELD="$2" -DZMK_CONFIG="$CONFIG_DIR" -Wno-dev > "$LOGFILE" 2>&1
    if [[ $? -eq 0 ]]
    then
        ...
        OUTPUT="$OUTPUT_DIR/$2-$1-zmk.$TYPE"
...
cd "$HOST_ZMK_DIR/app"
compile_zmk "nice_nano_v2" "corne_left"

if [[ $RIGHT = "true" ]]; then
    compile_zmk "nice_nano_v2" "corne_right"
